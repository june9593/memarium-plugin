...
describe("buildPreparePayload — consumed = episodic sourceSessions ∪ skip ledger", () => {
  ...
  const rel = (id: string) => `raw_sessions/claude/code-demo/2026-01-01/x__${id}.md`;
  const sess = (id: string) => ({
    sessionId: uuid(id), shortId: id, tool: "claude", project: "code-demo",
    projectRaw: "/work/code-demo", startedAt: "2026-01-01T00:00:00Z", endedAt: `2026-01-0${id.slice(1)}T00:00:00Z`,
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(repo, ".memarium/index.memory.json"), JSON.stringify({ version: 1, entries: {
      "episodic/code-demo/e1": memEntry({ id: "episodic/code-demo/e1", type: "episodic",
        path: "memory/episodic/code-demo/e1.md", sourceSessions: [uuid("s1")] }),
      "semantic/code-demo/f1": memEntry({ id: "semantic/code-demo/f1", type: "semantic",
        path: "memory/semantic/code-demo/f1.md", sourceSessions: [uuid("s3")] }),
    } }));
  ...
  it("digests only sessions not consumed by an episodic and not skip-ledgered; a semantic-only session stays PENDING", async () => {
    ...
    const p = buildPreparePayload({ project: "code-demo" });
  ...
  it("exposes only ACTIVE episodic ids as reuse hints (excludes superseded + pinned)", async () => {
    ...
    writeFileSync(join(repo, ".memarium/index.memory.json"), JSON.stringify({ version: 1, entries: {
      "episodic/code-demo/e1": memEntry({ id: "episodic/code-demo/e1", type: "episodic", path: "memory/episodic/code-demo/e1.md" }),
      "episodic/code-demo/old": memEntry({ id: "episodic/code-demo/old", type: "episodic", status: "superseded", path: "memory/episodic/code-demo/old.md" }),
      "episodic/code-demo/pin": memEntry({ id: "episodic/code-demo/pin", type: "episodic", status: "pinned", path: "memory/episodic/code-demo/pin.md" }),
    } }));
    const { buildPreparePayload } = await import("../../src/commands/prepare.js");
    const p = buildPreparePayload({ project: "code-demo" }) as unknown as Record<string, unknown>;
    expect(p.existingEpisodes).toEqual({ "code-demo": ["episodic/code-demo/e1"] });
