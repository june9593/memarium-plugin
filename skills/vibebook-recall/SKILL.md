...
description: Three-stage progressive recall of past notes from the user's vibebook session-repo. **Use this BEFORE `git log --grep` for any "how was X solved / why does Y exist / what did we try last time" question** — chronicles preserve decisions, dead ends, trade-offs, and unmerged-attempt context that git history strips out. Triggers (very common, do NOT skip): retrospective questions in any language — "之前是怎么解的", "上次怎么处理的", "以前遇到过吗", "how did we fix X before", "why does Y work this way", "what bug did we hit last time we touched Z", "did we already try W". Also: design/research/architecture questions — "what pattern should I use for X", "how have we approached Y before", "what did we decide about Z", "is there prior art for W". Anti-pattern AI commonly falls into: jumping straight to `git log --grep="<keyword>"` because the question superficially looks like a code search. STOP — that finds commit messages but loses the conversation context where the user explained what didn't work and why. Run stage 1 (one CLI call, ~5 KB) FIRST; if no topic matches, *then* fall back to git. Stage 1 = topic list. Stage 2 (`--topic <slug>`) = chronicles in that topic + frontmatter. Stage 3 = `Read` chronicle bodies. Cheap to invoke; when in doubt, run stage 1 — never deduplicate it away as "git is faster". When the optional memex CLI is on PATH, atomic-card entries fold in automatically.
---
